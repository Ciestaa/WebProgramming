...
class Post
{
			...
			if($this->error == ""){
				...
				if(isset($data['parent']) && is_numeric($data['parent'])){
					...
					if(is_array($mypost) && $mypost['userid'] != $userid){
						...
						//add notification
						add_notification($_SESSION['TravelBlog_userid'],"comment",$mypost);
					}
	...
	public function i_own_post($postid,$TravelBlog_userid)
	{
		...
		if(is_array($result)){
			...
			if($result[0]['userid'] == $TravelBlog_userid){
	...
	public function like_post($id,$type,$TravelBlog_userid){
		...
		$DB = new Database();
			...
			if(is_array($result)){
				...
				if(!in_array($TravelBlog_userid, $user_ids)){

					$arr["userid"] = $TravelBlog_userid;
					...
					if($type != "user"){
						...
						//add notification
						add_notification($_SESSION['TravelBlog_userid'],"like",$single_post);
					}

				}else{

					$key = array_search($TravelBlog_userid, $user_ids);
					unset($likes[$key]);
